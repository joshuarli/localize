...
`localize` is a maintenance toolkit for static HTML sites. Three subcommands:
...
- `src/cli.rs` — argument parsing (`lexopt`), file discovery (`glob` + `walkdir`; `jwalk` for `discover_and_index`), orchestrates all workflows. Contains `cmd_check` (scan + optional download/clean), `cmd_zap`, `cmd_towebp`, `discover_and_index` (single walk building both the HTML file list and canonical href set via parallel `jwalk`), `scan_all` (parallel HTML tokenization via `tokio`), `print_human`/`print_json` (unified output).

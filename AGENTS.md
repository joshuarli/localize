...
`localize` is a maintenance toolkit for static HTML sites. Six subcommands:
...
- **extract-css** — extract inline `<style>` CSS blocks into separate `.css` files and replace them with `<link rel="stylesheet">` references in `<head>`. Byte-exact CSS preservation (no minification). Naming from `/*# sourceURL=... */` comments, `id` attributes, or content hash. Two-step modification: delete all `<style>` blocks via descending-span surgery (same proven pattern as zap), then insert all `<link>` tags at a single anchor point. Dry-run by default, `--apply` to write. Full architecture in [`docs/CSS.md`](docs/CSS.md).
...
- `src/extract_css.rs` — inline CSS extraction. Uses `html5gum` to find `<style>` elements and their byte spans, extracts content byte-exact, generates filenames from `/*# sourceURL=... */` comments → `id` attributes → XXH3 content hash, and returns the structured result (files to write, link tags to insert, spans to delete). The CLI handles the two-step HTML modification: delete all `<style>` blocks via descending-span removal, then insert `<link rel="stylesheet">` tags at a single `<head>` anchor point. Full details in [`docs/CSS.md`](docs/CSS.md).
...
3. **extract-css**: discover HTML files → process in parallel (`crossbeam` thread scope) → for each file: tokenize with `html5gum` to find all `<style>` elements and their byte spans → extract CSS content byte-exact → generate filenames (sourceURL comment → id attribute → XXH3 content hash) → compute relative paths for `<link>` tags. When `--apply`: delete all `<style>` blocks via descending-span removal (same proven pattern as zap) → write .css files to `--css-dir` → insert `<link rel="stylesheet">` tags before `</head>` (with fallbacks for minified/HTML5-optional head markup) → write modified HTML via tmp+rename. Full details in [`docs/CSS.md`](docs/CSS.md).
